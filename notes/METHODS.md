...
| D26 | Targeted flip cap | fraction is of training rows (D8) but cannot exceed the number of malignant training rows (≈ 37 % of train). **At 40 % every malignant row is flipped** | Stated so the 40 % point is read correctly; report at 30 % if the cap is unwanted |
